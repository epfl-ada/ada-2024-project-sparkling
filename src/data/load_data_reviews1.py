...
IMDB_REVIEW_1 = "IMDBReviews1"
def load_movie_reviews1(df_movies, save_to_load_from="IMDBReviews1.csv"):
    """
    ...
    Argument:
        ...
        - save_to_load_from: File to save or load data from (None -> Load from parts and do not save) -> Default: IMDBReviews1.csv

    If the file 'IMDBReviews1/save_to_load_from' exists, then load it, otherwise load the data from the different parts,
    ...
    def loadfile(id):
        ...
        if not os.path.isfile(path):
            raise Exception(f"The file {file_name} is missing -> Please download the dataset from https://www.kaggle.com/datasets/ebiswas/imdb-review-dataset/data (See README)")
    ...
    for i in range(1, 7):
        ...
        df_data = df_data.drop(["review_id", "reviewer", "review_summary", "review_date", "spoiler_tag", "helpful"], axis=1)
        ...
        df_data["movie_name"] = df_data["movie"].str.replace(pattern_season_episode, '', regex=True)

        # Second format:
        pattern_episode = r" Episode #\d+.\d+ "
        df_data["movie_name"] = df_data["movie_name"].str.replace(pattern_episode, '', regex=True)
        ...
        pattern_year = r"(?::| )\((\d{4})(?:[^\)]+|)\)$"
        df_data["movie_year"] = df_data["movie_name"].str.extract(pattern_year).astype('Int64')
        df_data["movie_name"] = df_data["movie_name"].str.replace(pattern_year, '', regex=True)
