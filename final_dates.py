...
def date_choice(dates):
    ...
    return pd.Series(final_date) #pd.DataFrame(data={'release_year': [final_date[0]], 'release_month': [final_date[1]]})
...
def get_final_dates(df): 
    ...
    final_df = final_df.drop((final_df[np.isnan(final_df['release_year']) | np.isnan(final_df['release_month']) | (final_df['release_month']==0)]).index)
    return final_df